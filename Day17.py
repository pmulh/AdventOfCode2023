...
# Add on value of start point
start_node_value = data[start_node[0], start_node[1]]
for node in shortest_path.keys():
    shortest_path[node] += start_node_value

min_at_end_node = abs(sys.maxsize)
for node in shortest_path.keys():
    if (node[0] == nrows-1) and (node[1] == ncols-1):
        print(node, shortest_path[node])
        if abs(shortest_path[node]) < min_at_end_node:
