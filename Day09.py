...
total_sum = 0
for line in lines:#['-2 -7 -20 -48 -88 -114 -64 173 770 1975 4124 7654 13116 21188 32688 48587 70022 98309 134956 181676 240400']:#lines:
    values = line.split(' ')
    ...
    # Extrapolate
    # Append new value at end of row i equal to: current value at end of row i + value at end of row i+1
    ...
    for row_num in range(len(values_array)-1, -1, -1):
        ...
        if row_num == len(values_array) - 1:
            values_array[row_num].append(0)
            continue

        # Otherwise, append new value based on current value + value from next row
        new_value = values_array[row_num][-1] + values_array[row_num+1][-1]
        values_array[row_num].append(new_value)

    # Add extrapolated value (final value in first element in values_array) to total
    total_sum += values_array[0][-1]
    # print(line)
    # print(line)
    # print(total_sum)
